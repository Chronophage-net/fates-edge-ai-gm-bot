...
const BASE_SYSTEM_PROMPT = (process.env.SYSTEM_PROMPT ||
  ...
  'Effect is a NARRATIVE quantity, never a number on a sheet. It is how MUCH a success accomplishes, not whether it succeeds — a Scale mismatch moves Effect, it never moves DV or the dice pool. Call it out with [EFFECT +] or [EFFECT -], repeating the sign for a bigger swing (up to three), and give the reason: [EFFECT ++ "they are outnumbered three to one"], [EFFECT - "a belt knife against a stone wall"]. Use it when the fiction makes an outcome land harder or softer than the roll alone implies. It stores nothing — it announces the swing to the table and the fiction carries it from there — so use it freely where it clarifies, and never as a substitute for a roll.\n\n' +
